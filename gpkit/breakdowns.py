...
def get_model_breakdown(solution):
    ...
    for constraint, senss in solution["sensitivities"]["constraints"].items():
        ...
        constrstr = try_str_without(
            constraint, {"units", ":MAGIC:" + lineagestr(constraint)}
        )
...
def crawl_modelbd(bd, lookup, name="Model"):
    ...
    if bd:
        lookup[name] = tree
    for subname, subtree in sorted(
        bd.items(), key=lambda kv: (-float("%.2g" % kv[1]["|sensitivity|"]), kv[0])
    ):
        ...
        if len(pos_gtvks) != 1:  # we'll choose our favorite vk
            ...
            if candidatevks:
                ...
                chosenvk, *_ = sorted(
                    candidatevks,
                    key=lambda vk: (
                        -float("%.2g" % (gt.exp[vk] * vrisk.get(vk, 0))),
                        str(vk),
                    ),
                )
                for vk, pow in gt.exp.items():
...
def crawl(
        ...
        if (
            ...
            if factor != 1:
                ...
                if verbosity:
                    print(
                        "  " * indent
                        + "{ through a factor of %s (%s) }"
                        % (factor.str_without(["units"]), get_valstr(factor, solution))
                    )
                subsubtree = []
            ...
            if power != 1:
                ...
                transform = Transform(
                    1, 1 / power, keymon
                )  # inverted bc it's on the gt side
                orig_subtree.append(Tree(transform, basescale, subsubtree))
                orig_subtree = subsubtree
    ...
    ]  # ~10% of total last check # TODO: remove
    # sort by value, preserving order in case of value tie
    sortedmonvals = sorted(
        zip([-float("%.2g" % mv) for mv in monvals], range(len(mons)), monvals, mons)
    )
    ...
    for i, (_, _, scaledmonval, mon) in enumerate(sortedmonvals):
        ...
        if len(monsols) > 1 and verbosity:
            ...
            print(
                "  " * indent
                + "%s) forming %i%% of the RHS and %i%% of the total:"
                % (i + 1, scaledmonval / basescale * 100, scaledmonval * 100)
            )
            indent += 1
        ...
        if interesting_vks and (boring_vks or mag(mon.c) != 1):
            ...
            if (
                factor.units is None
                and isinstance(factor, FixedScalar)
        ...
        if (
            subkey is not None
            and subkey not in visited_bdkeys
            and subkey in bd
            and scaledmonval > 0.05
        ):
...
def discretize(tree, extent, solution, collapse, depth=0, justsplit=False):
    ...
    if any(v is None for v in values):
        bvs = zip(
            *sorted(
                (
                    (-float("%.2g" % v), i, b, v)
                    for i, (b, v) in enumerate(zip(branches, values))
                    if v is not None
                )
            )
        )
    ...
    if not all(extents):  # create a catch-all
        ...
        for subextent in reversed(extents):
            if not subextent or (branches[-1].value < miscval and surplus < 0):
                ...
                miscvkeys += vkeys
                surplus -= (
                    round(scale * (miscval + v)) - round(scale * miscval) - subextent
                )
                miscval += v
    ...
    if surplus:
        ...
        bump_priority = sorted(
            (ext, sign * float("%.2g" % b.value), i)
            for i, (b, ext) in enumerate(zip(branches, extents))
        )
...
def prune(tree, solution, maxlength, length=-1, prefix=""):
    ...
    for branch in branches:
        keylength = max(
            len(get_valstr(branch.key, solution, into="(%s)")),
        ...
        if branchlength > maxlength:
            return Tree(key, extent, [])
    return Tree(
        key, extent, [prune(b, solution, maxlength, length, prefix) for b in branches]
    )
    ...
    *,
    ...
    for pos in range(height):
        for depth in reversed(range(1, len(mt))):
            ...
            if key not in legend and (
                isinstance(key, tuple)
                or (depth != len(mt) - 1 and chararray[depth + 1, pos] != " ")
            ):
            ...
            if not isinstance(key, FixedScalar):
                labeled.add(keystr)
                if span > 1 and (
                    collapse or pos + 2 >= height or chararray[depth, pos + 1] == "┃"
                ):
            ...
            if (
                key in breakdowns
                and not chararray[depth + 1, pos].strip()
                and (depth >= len(mt) - 2 or not chararray[depth + 2, pos].strip())
            ):
                keystr = keystr + "╶⎨"
    ...
    if showlegend:  # create and print legend
        ...
        for key, shortname in sorted(legend.items(), key=lambda kv: kv[1]):
            legend_lines.append(
                legend_entry(
                    key, shortname, solution, prefix, basically_fixed_variables
                )
            )
    ...
    def get_tree(self, key, *, permissivity=2, verbosity=0):
        ...
        if isinstance(key, str):
            if key == "model sensitivities":
                ...
                keys = [
                    vk for vk in self.bd if key in str(vk) and key[-1] == str(vk)[-1]
                ]
                if not keys:
                    raise KeyError(key)
                elif len(keys) > 1:
                    raise KeyError(
                        "There are %i keys containing '%s'." % (len(keys), key)
                    )
                (key,) = keys
        if tree is None:
            tree = crawl(
