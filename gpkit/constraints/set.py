...
class ConstraintSet(list, ReprMixin):  # pylint: disable=too-many-instance-attributes
    ...
    def __repr__(self):
        ...
        return (
            "<gpkit.%s object containing %i top-level constraint(s)"
            " and %i variable(s)>"
            % (self.__class__.__name__, len(self), len(self.varkeys))
        )
...
class ConstraintSetView:
    ...
    def __getattr__(self, attr):
        ...
        if not hasattr(value, "shape"):
            raise ValueError(
                "attribute %s with value %s did not have"
...
def badelement(cns, i, constraint, cause=""):
    ...
    return ValueError(
        "Invalid ConstraintSet element '%s' %s was %s.%s"
