...
class Signomial(Nomial):
    ...
    def __init__(
        ...
        if not isinstance(hmap, NomialMap):
            if hasattr(hmap, "hmap"):
                ...
                raise ValueError(
                    "Nomial construction accepts only NomialMaps,"
    ...
    def sub(self, substitutions, require_positive=True):
        ...
        """
        return Signomial(
            self.hmap.sub(substitutions, self.vks), require_positive=require_positive
        )
    ...
    def __init__(self, left, right):
        ...
        if self.unsubbed and len(self.vks) > 1:
            ...
            for key, e in exp.items():
                ...
                lbs = frozenset(
                    (k, "lower" if np.sign(e) != s_e else "upper")
                    for k, e in exp.items()
                    if k != key
                )
...
class SignomialInequality(ScalarSingleEquationConstraint):
    ...
    def __init__(self, left, oper, right):
        ...
        if not SignomialsEnabled:
            raise TypeError(
                "Cannot initialize SignomialInequality"
    ...
    def as_hmapslt1(self, substitutions):
        ...
        self._coeffsigs = {
            exp: Signomial(hmap, require_positive=False)
            for exp, hmap in posy_hmaps.items()
        }
        self._sigvars = {
            exp: (list(self._negysig.vks) + list(sig.vks))
